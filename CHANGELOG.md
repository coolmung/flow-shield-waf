...
-   新增：可配置最大上传 / 请求体限制（默认 50 MB），以及源站响应超时时间（默认 60 秒）
-   防护日志筛选支持按请求 ID 精确查找；所有经流盾转发或拦截的请求都会带上 `X-WAF-Request-Id`（源站请求头与客户端响应头）
-   IP 组管理以及匹配规则中均新增对 IPv6 的支持
-   一键安装 / 更新脚本在构建完成后自动执行 `docker builder prune -f` 与 `docker image prune -f`，自动清理未使用的构建缓存与悬空镜像，避免多次升级后磁盘占用持续增长

### 变更

-   重新设计系统设置页面，按六个分组组织（账户、引擎、访客页面、日志采样、面板与通道、配置备份），提供更人性化的操作体验
-   优化引擎自身错误页（413、502 等）的响应展示效果：统一为错误码、简要说明与流盾通用页脚，保留原始 HTTP 状态码；与 WAF 规则拦截页互不影响
-   优化 AI 防护「AI 配置」页面的 UI 显示效果，配置分组与控件类型与系统设置页保持一致
-   引擎对大体积请求不再把整包读入 Lua：`http.body.raw` / `json` 只看前 64KB 前缀（大 `multipart` 仍可匹配内置 SQLi 等 body 规则）；`http.body.form` 仅在小体积非 multipart 时整表解析，大包从相同前缀解码。仅当启用规则使用 `http.upload.*` 时才提取文件名/后缀（前 64KB，且不再把 `filename*` 误当成 `filename=`）
-   AI 对话确认创建自定义规则后，可在消息中查看仍存在的规则
-   AI 创建的自定义规则名称自动加上 `[AI规则]` 前缀
-   AI 防御分析最多轮次由 6 轮调整为 10 轮
-   AI 写入自定义规则与人工 API 对齐：非观察模式禁止空条件，并校验站点与引用
-   AI 防护默认规则应用模式改为「自动分析并处理」：由 AI 在观察、拦截、JS 挑战、滑动验证中选择动作（原「自动创建拦截」并入该模式）
-   AI 对话可开关自定义规则，并可创建与管理 Bot 库、IP 组；联网搜索会脱敏查询并隔离外部指令，自动防护仅在管理员显式开启后可用
-   优化程序启动顺序获取和逻辑，避免出现可能因 Nginx 未启动而导致的警告

### 修复

-   修复后台站点列表在批量删除或批量修改后，数据不刷新的问题
-   修复 AI 防护在「仅生成建议」模式下点击应用/邮件应用后显示已应用但未真正创建规则的问题
-   修复导入「系统设置」后未重载引擎，导致最大上传限制与源站超时未写入 Nginx 的问题
-   修复引擎 snippet 写入失败仍提示重载成功、设置页随后弹出「已保存」的问题；镜像缺少 snippet 时启动脚本会写入默认 50m / 60s

## [1.0.6] - 2026-08-18

### 新增

-   防护日志记录 **直连 IP**（TCP 连接地址）。启用 CDN 真实客户端 IP 后，日志详情与筛选仍可看到边缘节点地址；规则条件新增「直连 IP」（`ip.tcp`）
-   证书「到期前自动续期」可配置续期成功后自动推送到宝塔 / 1Panel 所选站点；编辑已有证书时可用「立即同步」测试

### 变更

-   证书管理列表增加「到期自动续期」列：已开启显示绿色标签并注明证书机构，未开启显示关闭
-   证书管理点击名称可打开更新、删除菜单，交互与防护规则一致
-   站点管理筛选栏右侧可在卡片与表格之间切换；默认卡片，选择会记在浏览器本地
