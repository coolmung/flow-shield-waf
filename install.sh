...
write_env_file() {
  ...
  local redis_pw jwt challenge entrance
...
# 面板安全入口。未登录必须访问 http://IP:PORT/<此值> 才打开登录页；留空则直接进入登录页
...
print_success() {
  ...
  local title panel_port host_hint host_ip panel_url entrance
  panel_port="$(grep -E '^PANEL_PORT=' .env 2>/dev/null | cut -d= -f2 | tr -d '\r' || true)"
  panel_port="${panel_port:-9000}"
  entrance="$(grep -E '^PANEL_ENTRANCE=' .env 2>/dev/null | cut -d= -f2- | tr -d '\r' | sed 's#^/##;s#/$##' || true)"
  host_hint="<服务器IP>"
  ...
  ui_row "面板  ${panel_url}"
  if [[ -n "$entrance" ]]; then
    ui_row "入口  /${entrance}  （请妥善保存；未登录时只能使用此地址访问面板）"
  fi
